...
public class sxe140630_Assignment1 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		Node root = LearnTree(trainingrecords,unusedattributesnumbers);
		
		// printing the tree.
		
		printTree(root,0);
		
		// Comparing the formed tree with the train data.
		double accuracytrain = calculateaccuracy(root, trainingrecords);
		System.out.println("Accuracy on training set ("+trainingrecords.size()+" instances): "+accuracytrain);
		double accuracytest = calculateaccuracy(root,testingrecords);
		System.out.println("Accuracy on testing set ("+testingrecords.size()+" instances): "+accuracytest);
	...
	private static void printTree(Node root,int no_of_spaces) {
		...
		if(root.getChildren().size()!=0)
		{
			...
			String attrname = sxe140630_Assignment1.attributeslist.get(attrno);
	...
	private static Node LearnTree(ArrayList<Record> trainingrecords, ArrayList<Integer> unusedattributesnumbers) {
		...
		if(unusedattributesnumbers.isEmpty())
		{
			...
			else
			{
				...
				if(sxe140630_Assignment1.no_of_zero_attributes > sxe140630_Assignment1.no_of_one_attributes)
				{
					root.setClassnumber(0);
				}
				else 
